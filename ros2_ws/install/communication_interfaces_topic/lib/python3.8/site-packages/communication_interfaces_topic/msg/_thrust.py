    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'communication_interfaces_topic.msg.Thrust')
            logger.debug(
...
class Thrust(metaclass=Metaclass_Thrust):
    ...
    def __init__(self, **kwargs):
        ...
        if 'thrust_data' not in kwargs:
            self.thrust_data = numpy.zeros(12, dtype=numpy.float32)
        else:
            self.thrust_data = numpy.array(kwargs.get('thrust_data'), dtype=numpy.float32)
            assert self.thrust_data.shape == (12, )
    ...
    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if all(self.thrust_data != other.thrust_data):
            return False
        return True
    ...
    @property
    def thrust_data(self):
        """Message field 'thrust_data'."""
        return self._thrust_data

    @thrust_data.setter
    def thrust_data(self, value):
        if isinstance(value, numpy.ndarray):
            assert value.dtype == numpy.float32, \
                "The 'thrust_data' numpy.ndarray() must have the dtype of 'numpy.float32'"
            assert value.size == 12, \
                "The 'thrust_data' numpy.ndarray() must have a size of 12"
            self._thrust_data = value
            return
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 True), \
                "The 'thrust_data' field must be a set or sequence with length 12 and each value of type 'float'"
        self._thrust_data = numpy.array(value, dtype=numpy.float32)
