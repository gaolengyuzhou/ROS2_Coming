...
void IMU::Euler2Quaternion(float roll, float pitch, float yaw, imu_quaternion_t &q)
{
...
{
    imu.angle_euler.x = mpu_->getAngleX() * M_PI / 180.0;// X角度
    imu.angle_euler.y = -mpu_->getAngleY() * M_PI / 180.0;
    imu.angle_euler.z = mpu_->getAngleZ() * M_PI / 180.0;

    imu.angular_velocity.x = mpu_->getAccAngleX();// X轴角加速度
    imu.angular_velocity.y = -mpu_->getAccAngleY();
    imu.angular_velocity.z = mpu_->getGyroZ();

    imu.linear_acceleration.x = mpu_->getAccX();//X轴加速度
